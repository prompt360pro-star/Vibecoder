...
type PostTypeOption = {
  id: 'ACHIEVEMENT' | 'PROMPT' | 'HELP' | 'GENERAL'
  label: string
  emoji: string
}
  ...
  const handlePublish = () => {
    ...
    createPostMutation.mutate(
      ...
      },
      {
        ...
        onError: () => {
          Alert.alert('Erro', 'Nao foi possivel publicar. Tenta novamente.')
        },
      },
    )
  }
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          >
            {POST_TYPES.map((postType) => {
              const isActive = type === postType.id
              return (
                <Pressable
                  key={postType.id}
                ...
                >
                  <Text style={[styles.typeEmoji, !isActive && styles.typeEmojiInactive]}>
                    {postType.emoji}
                  </Text>
          ...
          <View style={styles.actionRow}>
            ...
            >
              <Ionicons
                name="code-slash"
                size={20}
                color={showCode ? COLORS.accentPurple : COLORS.textMuted}
              />
              <Text style={[styles.actionToggleText, showCode && styles.actionToggleTextActive]}>
                {showCode ? 'Remover Codigo' : 'Adicionar Codigo'}
              </Text>
            </Pressable>
          </View>
          ...
          {showCode ? (
            <View style={styles.codeInputWrapper}>
              <View style={styles.codeHeader}>
                ...
                <Text style={styles.codeHeaderText}>Codigo</Text>
              </View>
              <TextInput
                style={styles.codeInput}
                multiline
                placeholder="Cola o teu codigo aqui..."
                placeholderTextColor={COLORS.textMuted}
                value={codeBlock}
          ...
          <View style={styles.tagsSection}>
            <Text style={styles.tagsLabel}>Tags (Minimo 0, Max 5) {selectedTags.length}/5</Text>
...
const styles = StyleSheet.create({
  ...
  publishBtnDisabled: { backgroundColor: 'rgba(139, 92, 246, 0.3)' },
  publishBtnText: { color: COLORS.textPrimary, fontSize: 13, fontWeight: '700' },
  ...
  typeEmojiInactive: { opacity: 0.5 },
  typeLabel: { color: COLORS.textMuted, fontSize: 11, fontWeight: '700' },
  typeLabelActive: { color: COLORS.accentPurple, fontWeight: '700' },
  input: {
    backgroundColor: COLORS.bgCard,
    borderRadius: 12,
    padding: 16,
  ...
  actionToggleTextActive: { color: COLORS.accentPurple },
  codeInputWrapper: {
